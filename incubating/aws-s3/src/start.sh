AWS_ACCESS_KEY_ID=$(codefresh get context amazon --decrypt -o json |jq --raw-output '.spec.data.auth.jsonConfig.accessKeyId')
AWS_SECRET_ACCESS_KEY=$(codefresh get context amazon --decrypt -o json |jq --raw-output '.spec.data.auth.jsonConfig.secretAccessKey')
